...
- Run `./gradlew testDebugUnitTest` before shipping. The Drive merge logic (`mergeRows`) rewrites
  the user's real CSV in place, so it is covered by unit tests — add one for any change to how
  rows are merged, replaced, or formatted.
...
- **Before changing data, back up the Drive CSV, then verify the result against the backup**, not
...
- **Samsung exports are one-shot.** The app parses a `samsunghealth_*` folder in Downloads and then
  ...
  first if you'll need them for comparison. A large export takes ~10 minutes to *delete* (the system
  does per-file work inside the single delete call); that runs as its own background task and is
  expected, not a hang.
