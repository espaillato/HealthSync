...
class SyncWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    ...
    override suspend fun doWork(): Result = mutex.withLock {
        ...
        val exportCleanupNeeded = SamsungHealthExportImporter.stageAvailableExports(applicationContext, owner)
        ...
        return try {
            ...
            if (allRows.isNotEmpty()) {
                ...
                // this one step leaves syncState completely untouched below -- the Health Connect
                // cursors stay where they were (they're only committed after an upload, see
                // commitCursors), so those rows get re-read next time, and the Samsung-export
                // rows (already safely staged in PendingImports by this point regardless of what
                // happens here) simply wait for the next sync that does have Wi-Fi, rather than
                // this attempt being recorded as either a success (the data isn't actually on
                // Drive yet) or a failure (nothing actually went wrong).
            ...
            // Only now -- after the upload -- do the Health Connect cursors actually move. They
            // used to advance inside the read itself, so a skipped or failed upload permanently
            // dropped every Health Connect row it had read (nothing stages those for retry).
            reader.commitCursors()

            // Health Connect's per-record-type cursors were queued inside readSince (advanced on
            // success, or on a failure isolated to one record type forced forward a bounded
            // amount) and committed just above. Health Connect write sources can backfill
            // *already-passed* timestamps -- e.g. Samsung Health, the moment it's first granted
            // write access, wrote several hours of that day's heart-rate history retroactively,
            // and whole days of watch data arrive hours after the nightly sync has passed them --
            // which is why each record type's cursor only ever advances on its own success, never
            // gets pushed forward by some other metric's success, never moves backward, and why
            // every read re-covers a trailing window behind it (see HealthConnectReader).
            ...
            // Whether or not the upload worked -- the export's rows are already saved on disk, and
            // the raw export shouldn't sit in Downloads longer than necessary. Queued only now,
            // after the network work, because deleting a large export makes the phone sluggish
            // enough that an upload running alongside it timed out (seen live, twice in a row).
            if (exportCleanupNeeded) ExportCleanupWorker.enqueue(applicationContext)
        }
    }
