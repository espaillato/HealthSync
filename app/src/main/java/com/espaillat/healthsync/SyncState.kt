...
class SyncState(context: Context) {
    ...
    /**
     * Names of Samsung export folders that have been fully parsed and staged but not yet deleted
     * -- see SamsungHealthExportImporter. Lets the (slow, interruptible) delete resume on its own
     * without the folder ever being parsed twice.
     */
    fun processedExportFolders(): Set<String> = prefs.getStringSet(KEY_PROCESSED_EXPORTS, emptySet())?.toSet() ?: emptySet()

    fun isExportProcessed(name: String): Boolean = name in processedExportFolders()

    fun markExportProcessed(name: String) {
        prefs.edit().putStringSet(KEY_PROCESSED_EXPORTS, processedExportFolders() + name).apply()
    }

    fun unmarkExportProcessed(name: String) {
        prefs.edit().putStringSet(KEY_PROCESSED_EXPORTS, processedExportFolders() - name).apply()
    }
     ...
     * label it was always displayed as (`"Alex"`) -- every read of it went through the enum's
