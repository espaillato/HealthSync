...
object SamsungHealthExportImporter {
    ...
    private const val METRIC_DAILY_ACTIVITY = "daily_steps_distance"
    private const val METRIC_DAILY_CALORIES = "daily_calories"
     ...
     *
     * Returns true when at least one already-parsed export folder is waiting to be deleted. The
     * caller queues [ExportCleanupWorker] -- after its own upload, not here, so the slow delete
     * never runs alongside the network work it would otherwise compete with.
     */
    fun stageAvailableExports(context: Context, owner: String): Boolean {
        val syncState = SyncState(context)
        val folderUriString = syncState.samsungHealthExportFolderUri ?: return false
        ...
        for (exportFolder in exportFolders) {
            ...
            // Already parsed and staged by an earlier run whose delete never finished (deleting a
            // folder this size can outlast a background job) -- nothing left to parse, and parsing
            // a half-deleted folder would silently skip whatever files were already gone.
            if (syncState.isExportProcessed(exportFolderName)) {
                cleanupNeeded = true
                continue
            }

            // The whole export folder gets deleted afterwards -- not just the specific CSV/JSON
            // files this importer reads. Samsung's export dumps dozens of internal data types we
            // never touch (sleep detail, location, workout GPS tracks, etc.), and none of it
            // should linger in Downloads once it's been read: it's real personal health data
            // sitting in plain files. The delete is deliberately a single call on the folder, not
            // file by file -- and it is still slow, because the system's own storage layer does
            // per-file work inside that one call (a permission-revoke callback for every deleted
            // file, on a single thread). That cost can't be avoided or parallelized from here, so
            // instead of paying it inside the sync it's handed to [ExportCleanupWorker], which is
            // resumable and off the sync's clock.
            //
            // An unexpected failure mid-parse still ends in the folder being cleaned up: each
            // export is a full re-dump, so a metric that didn't get read simply gets covered by
            // the next one (its cursor never moved), and a bug in this importer should never be
            ...
            // Order matters: rows are saved first, the cursors that cover them second, the
            // "processed" marker last. Interrupted at any point the worst case is the folder being
            // parsed again (rows dedup by ID), never a cursor that has moved past rows that were
            // never saved.
            ...
            Log.i(
                TAG,
                "Parsed Samsung Health export '$exportFolderName' in ${(System.currentTimeMillis() - parseStarted) / 1000}s: " +
                    "${parsed.rows.size} rows staged" +
                    (if (parsed.anyFailure) " (at least one metric type failed to parse -- see the warning above)" else "")
            )
        }
    ...
    /**
     * Deletes every export folder already parsed and staged (see [stageAvailableExports]), each
     * with one call on the whole folder. Returns true once none are left. Safe to call repeatedly
     * and safe to be interrupted: a folder that's partly gone is simply deleted again, and it's
     * never parsed a second time because of the "processed" marker.
     */
    fun deleteProcessedExports(context: Context): Boolean {
        val syncState = SyncState(context)
        val pending = syncState.processedExportFolders()
        if (pending.isEmpty()) return true
        // No folder configured at all -- nothing this can ever act on, so don't ask to be retried.
        val folderUriString = syncState.samsungHealthExportFolderUri ?: return true
        val root = runCatching { DocumentFile.fromTreeUri(context, Uri.parse(folderUriString)) }.getOrNull()
        if (root == null || !root.isDirectory) {
            Log.w(TAG, "Export folder not accessible for cleanup (permission revoked or folder moved)")
            return false
        }

        val present = findExportFolders(root, maxDepth = 3).associateBy { it.name ?: "" }
        var allGone = true
        for (name in pending) {
            val folder = present[name]
            if (folder == null) {
                syncState.unmarkExportProcessed(name)   // already gone
                continue
            }
            val started = System.currentTimeMillis()
            val deleted = runCatching { folder.delete() }.getOrDefault(false)
            if (deleted) {
                syncState.unmarkExportProcessed(name)
                Log.i(TAG, "Deleted Samsung Health export '$name' in ${(System.currentTimeMillis() - started) / 1000}s")
            } else {
                allGone = false
                // Loud, not routine -- a folder full of real health data is still in Downloads.
                Log.e(TAG, "FAILED to delete Samsung Health export '$name' -- it still contains raw personal health data; will retry")
            }
        }
        return allGone
    }
    ...
    private fun parseExportFolder(
        ...
        // stops background jobs at about ten minutes, and a large export gets close -- had already
        // advanced some metrics' cursors without their rows ever being staged, and the next run
        // would then skip exactly the rows that were lost.
        val cursorUpdates = mutableMapOf<String, Instant>()
        ...
        // Steps, distance and calories are the metrics where the export is the authority and
        // Health Connect only a provisional stand-in -- see parseDailyActivity. Each has its own
        // cursor like every other metric here, so a repeat export only touches days newer than
        // the last one that was reconciled.
        run(METRIC_DAILY_ACTIVITY, "com.samsung.shealth.step_daily_trend") { file, cursor ->
            parseDailyActivity(context, file, owner, cursor)
        }
        run(METRIC_DAILY_CALORIES, "com.samsung.shealth.calories_burned.details") { file, cursor ->
            parseDailyCalories(context, file, owner, cursor)
        }
    ...
    /**
     * Recovers the real name behind a custom exercise (e.g. a user-named bodyweight routine) -- confirmed real,
     ...
     * (confirmed against every real session of that exercise checked), matching what Health Connect's
    ...
    private fun parseBloodPressure(context: Context, file: DocumentFile, owner: String, cursor: Instant?): ParsedPart {
        ...
        for (row in dataRows) {
            ...
            // `start_time` here is already a UTC instant, despite the `time_offset` field beside
            // it that looks exactly like every other Samsung export type's naive-local-plus-
            // offset convention -- the same quirk the exercise fields have. Confirmed against
            // real data: Health Connect's own copy of the same reading matches the raw string to
            // the second, and read as "local time plus offset" the export-sourced readings piled
            // up between midnight and 6am (implausible for a deliberate BP check), while shifted
            // by the offset they land in daytime alongside the PDF-import readings. Parsed
            // directly as UTC, then converted to the reading's own local wall-clock time (via the
            // row's recorded offset) only where that's actually needed -- the ID below.
            val instant = parseLocalDateTimeAsUtc(startRaw) ?: continue
            if (cursor != null && !instant.isAfter(cursor)) continue
            val day = instant.atZone(ZoneId.systemDefault()).toLocalDate()
            if (!day.isBefore(today)) continue

            // The ID has to be the reading's own *local* wall-clock date and time -- matching the
            // PDF importer's scheme exactly, since that's the whole point (cross-source dedup,
            // see this function's doc). The recorded offset is used rather than the processing
            // phone's current zone, which may not be the zone the reading was taken in.
            val recordedOffset = row.valueOf(header, "com.samsung.health.blood_pressure.time_offset")
                ?.let { UTC_OFFSET_PATTERN.find(it) }
                ?.let { m -> runCatching { ZoneOffset.of("${m.groupValues[1]}:${m.groupValues[2]}") }.getOrNull() }
                ?: ZoneId.systemDefault().rules.getOffset(instant)
            val localDateTime = instant.atOffset(recordedOffset).toLocalDateTime()
    ...
    /**
     * Daily steps and distance -- the export's copy is authoritative, Health Connect's is not.
     * Samsung Health writes one whole-day total into Health Connect and Health Connect's own
     * on-phone counter adds ~100 small records of steps already inside it; the export's
     * `step_daily_trend` has the same number Samsung Health itself displays as its "combined"
     * source (`source_type` -2, one row per day -- the other two are the phone and watch
     * separately, which overlap it). Rows are flagged replaceExisting so they correct whatever
     * Health Connect already put on Drive for the day rather than being dropped as duplicates,
     * and they carry the same `steps_daily_<date>` / `distance_daily_<date>` IDs as the Health
     * Connect rows for exactly that reason.
     *
     * Only *settled* days are written -- strictly older than [HealthConnectReader.TRAILING_DAYS]
     * ago. A day's total keeps being revised for a little while as the watch and phone finish
     * syncing, and Health Connect's own trailing window keeps a provisional value for those
     * recent days up to date; the export takes over once a day has stopped moving, so the two
     * never both claim one day. The per-metric cursor then only ever advances through settled
     * days, so a later export re-reads nothing older than what's already been reconciled.
     */
    private fun parseDailyActivity(context: Context, file: DocumentFile, owner: String, cursor: Instant?): ParsedPart {
        val (header, dataRows) = readSamsungCsv(context, file)
        if (header.isEmpty()) return ParsedPart(emptyList(), null)

        val settledBefore = LocalDate.now(ZoneId.systemDefault()).minusDays(HealthConnectReader.TRAILING_DAYS)
        val rows = mutableListOf<CsvRow>()
        var maxInstant: Instant? = null
        for (row in dataRows) {
            if (row.valueOf(header, "source_type") != "-2") continue
            val day = parseDay(row.valueOf(header, "day_time")) ?: continue
            if (!day.isBefore(settledBefore)) continue
            val ts = day.atStartOfDay(ZoneOffset.UTC).toInstant()
            if (ts.isBefore(EARLIEST_PLAUSIBLE_SAMSUNG_DATA)) continue
            if (cursor != null && !ts.isAfter(cursor)) continue

            val steps = row.valueOf(header, "count")?.toDoubleOrNull()
            val distance = row.valueOf(header, "distance")?.toDoubleOrNull()
            // A zero-step day is a day the watch wasn't tracking, not a real total of zero.
            if (steps == null || steps <= 0.0) continue
            rows += CsvRow(ts, owner, "steps", Math.round(steps).toString(), "count", "steps_daily_$day", replaceExisting = true)
            if (distance != null) {
                rows += CsvRow(ts, owner, "distance", formatValue(distance), "meters", "distance_daily_$day", replaceExisting = true)
            }
            if (maxInstant == null || ts.isAfter(maxInstant)) maxInstant = ts
        }
        return ParsedPart(rows, maxInstant)
    }

    /**
     * Daily active and total calories, from Samsung's own per-day breakdown: `active_calorie` is
     * what Samsung Health shows as active kcal, and `rest_calorie` is the resting (basal) burn,
     * so the day's total is their sum (`tef_calorie` -- thermic effect of food -- is included
     * when present, and is zero in every row seen so far). Same authority, ID scheme, replace
     * behaviour and settling rule as [parseDailyActivity]. Health Connect's own
     * `total_calories_burned` on this account matches none of these figures (it ran well below
     * even the active number on some days), and its `active_calories_burned` has no data at all,
     * so this is also what populates that metric for the first time.
     */
    private fun parseDailyCalories(context: Context, file: DocumentFile, owner: String, cursor: Instant?): ParsedPart {
        val (header, dataRows) = readSamsungCsv(context, file)
        if (header.isEmpty()) return ParsedPart(emptyList(), null)

        val prefix = "com.samsung.shealth.calories_burned."
        val settledBefore = LocalDate.now(ZoneId.systemDefault()).minusDays(HealthConnectReader.TRAILING_DAYS)
        val rows = mutableListOf<CsvRow>()
        var maxInstant: Instant? = null
        for (row in dataRows) {
            val day = parseDay(row.valueOf(header, prefix + "day_time")) ?: continue
            if (!day.isBefore(settledBefore)) continue
            val ts = day.atStartOfDay(ZoneOffset.UTC).toInstant()
            if (ts.isBefore(EARLIEST_PLAUSIBLE_SAMSUNG_DATA)) continue
            if (cursor != null && !ts.isAfter(cursor)) continue

            val active = row.valueOf(header, prefix + "active_calorie")?.toDoubleOrNull()
            val rest = row.valueOf(header, prefix + "rest_calorie")?.toDoubleOrNull()
            val tef = row.valueOf(header, prefix + "tef_calorie")?.toDoubleOrNull() ?: 0.0
            if (active != null) {
                rows += CsvRow(ts, owner, "active_calories_burned", formatValue(active), "kcal", "active_calories_burned_daily_$day", replaceExisting = true)
            }
            if (active != null && rest != null && rest > 0.0) {
                rows += CsvRow(ts, owner, "total_calories_burned", formatValue(active + rest + tef), "kcal", "total_calories_burned_daily_$day", replaceExisting = true)
            }
    ...
    private fun parseDay(raw: String?): LocalDate? = raw?.take(10)?.let { runCatching { LocalDate.parse(it) }.getOrNull() }
