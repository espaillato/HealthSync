...
./gradlew testDebugUnitTest # JVM unit tests (CSV merge/replace logic, timestamp format and migration)
```
...
Every row's `timestamp_local` uses one format — local wall-clock time with an explicit numeric
offset, whole seconds, e.g. `2026-09-20T00:00:00+09:00` — so a reader never has to work out whether
a given row is UTC or local. A daily-aggregated row is stamped local midnight of the day it covers,
so the date portion always matches the day a human actually experienced; a point-in-time row (a
weigh-in, a workout, a blood-pressure reading) carries its own local time. Files written by older
versions used a `timestamp_utc` column (real instants in UTC, with daily rows as a fake UTC
midnight); the first time such a file is next written the app converts it in place, assuming the
history was recorded in the phone's current time zone.

### Overlapping sources, late data, and reconciling with the export

Several apps can write the *same* activity into Health Connect, so summing every record overstates
it. Confirmed on real data: Samsung Health writes one whole-day steps total, and Health Connect's
own on-phone step counter adds ~100 small records of steps that are already inside it — summing
both inflated days by 40–80%, and Health Connect's aggregate call only removes the records that
overlap in time, so it doesn't fix this. For the additive metrics (steps, distance, elevation,
floors, calories) each day's value is therefore the **single data origin with the largest total**,
not the sum across origins. Treat that as a good provisional figure, not the final word.

Two things keep daily figures honest after the fact:

- **Trailing window.** Every sync re-reads the last 3 days behind each cursor, and a daily row from
  those days may be *revised in place* if its value changed. Whole days of watch data can reach
  Health Connect hours after the nightly sync has already passed them, so a cursor that only moves
  forward would lose them. Cursors are also only committed after the upload succeeds, so a skipped
  or failed upload re-reads the same window instead of silently dropping it.
- **Export reconciliation** (Samsung only). For steps, distance and active/total calories the
  Samsung Health export is the authority: its combined daily figures replace whatever Health
  Connect put on Drive, and fill days Health Connect never had. Only *settled* days (older than the
  trailing window) are written, each metric has its own cursor so a repeat export only touches days
  newer than the last one, and a replaced value is only ever written when it actually differs.
...
temperature, and custom exercise names**. The export is also the authority for **daily steps,
distance and calories** (see [Overlapping sources](#overlapping-sources-late-data-and-reconciling-with-the-export)
above): those do reach Health Connect, but overcounted. The only way back in is Samsung Health's own full
...
instead of inside the sync, so a slow delete never delays or endangers the upload.
...
- Fresh install → enter a name → import key → grant permissions → sync → the CSV shows up in
  ...
  `timestamp_local,owner,metric,value,unit,source_record_id`, header included.
